...
class AudioRecorderExample(QWidget):
    ...
    def init_ui(self):    
        ...
        self.selected_file_label = QLabel('ㅎㅇ')
    ...
    # 초기에 필요없는 요소 숨김
        ...
        # 초기에 일부 버튼 비활성화
        self.set_buttons_enabled(['음악 분할', '음성 녹음', '음성 불러오기', '합성및 저장'], True)
...
# 설명서
    ...
    def explanation_button(self):
        ...
        self.mp3_wav_load_button.hide()
        self.split_check_button.hide()
        # self.set_buttons_enabled(['음성 녹음', '음성 불러오기', '합성및 저장'], True)
    ...
    def spleeter_start(self):
        ...
        if self.extract_audio_stream(in_file, out_file):
            try:
                ...
                command = f"MusicSpliter.exe {out_file} {out_path}"
                ...
                QMessageBox.warning(self, "오류", "MusicSpliterHead.exe 실행 중 오류 발생: " + str(e), QMessageBox.Ok)
...
# 음성 녹음 기능
    ...
    def stop_record(self):
        ...
        if self.audio_recorder.state() == QAudioRecorder.RecordingState:
            ...
            self.button_out(True)
            # self.voice_play_button.setEnabled(True)
            # self.simultaneous_play_voice_button.setEnabled(True)
            self.media_player.setMedia(QMediaContent())
    ...
    def play_record(self):
        try:
            ...
            out_path = os.path.join(os.getenv('USERPROFILE'), 'Desktop', 'MusicProject')
            record_file_path = os.path.join(out_path, "recording.wav")   
